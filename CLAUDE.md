...
**Current Version**: `1.0.6-alpha.1` (defined in `manifest.json` and `const.py`)
...
- **External API package** (`violet-poolController-api==0.0.15` on PyPI) - The HTTP client and low-level utilities are **not** in this repo; they ship as an external package. Provides:
...
- `violet-poolController-api==0.0.15` - External API client package (installed by HA automatically)
