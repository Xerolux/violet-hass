...
from .const import (
    DOMAIN, CONF_API_URL, CONF_POLLING_INTERVAL, CONF_USE_SSL, CONF_DEVICE_ID, CONF_USERNAME,
    CONF_PASSWORD, DEFAULT_POLLING_INTERVAL, DEFAULT_USE_SSL, API_READINGS
)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator = VioletDataUpdateCoordinator(hass=hass, config=config, session=session)
...
class VioletDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        self, hass: HomeAssistant, config: Dict[str, Any], session: aiohttp.ClientSession
    ) -> None:
    ...
    async def _async_update_data(self) -> Dict[str, Any]:
        """Fetch data from the Violet Pool Controller API with retries using exponential backoff."""
        for attempt in range(self.retries):
            try:
                ...
                _LOGGER.error(
                    f"Attempt {attempt + 1}/{self.retries} - Error while fetching data: {err}"
                )
                if attempt + 1 == self.retries:
                    raise UpdateFailed(
                        f"Error after {self.retries} attempts (Device ID: {self.device_id}, "
                        f"URL: {url}): {err}"
                    )
