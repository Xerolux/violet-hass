...
from homeassistant.helpers import aiohttp_client, entity_registry, config_validation as cv
...
PLATFORMS: List[str] = ["sensor", "binary_sensor", "switch", "climate", "cover", "number"]
_LOGGER = logging.getLogger(__name__)

async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    """Migrate old config entry."""
    _LOGGER.debug("Migrating from version %s", config_entry.version)
    return config_entry.version == 1  # Current version, no migration needed
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    config = {
        ...
        "password": "***" if entry.data.get(CONF_PASSWORD) else "",
        "device_name": entry.data.get(CONF_DEVICE_NAME, "Violet Pool Controller"),
        "polling_interval": int(entry.options.get(CONF_POLLING_INTERVAL, entry.data.get(CONF_POLLING_INTERVAL, DEFAULT_POLLING_INTERVAL))),
        "timeout_duration": int(entry.options.get(CONF_TIMEOUT_DURATION, entry.data.get(CONF_TIMEOUT_DURATION, DEFAULT_TIMEOUT_DURATION))),
        "retry_attempts": int(entry.options.get(CONF_RETRY_ATTEMPTS, entry.data.get(CONF_RETRY_ATTEMPTS, DEFAULT_RETRY_ATTEMPTS))),
        "active_features": entry.options.get(CONF_ACTIVE_FEATURES, entry.data.get(CONF_ACTIVE_FEATURES, [])),
    }
    _LOGGER.info("Config (entry_id=%s): %s", entry.entry_id, config)
...
def register_services(hass: HomeAssistant) -> None:
    ...
    if hass.services.has_service(DOMAIN, "set_temperature_target"):
        _LOGGER.debug("Services for %s already registered", DOMAIN)
        return
        ...
        for entity_id in call.data.get("entity_ids", []):
            ...
            try:
                ...
                if call.data.get("duration", 0) > 0:
                    hass.async_create_task(delayed_turn_off(entity_id, call.data["duration"]))
            except Exception as err:
